...
function SignupInner() {
  ...
  const roleParam = (params.get('role')==='vendor'?'vendor':'customer') as UserRole;
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-orange-100/60 overflow-hidden">
          ...
          <div className="px-8 py-8">
            <div className="grid grid-cols-2 gap-3 mb-6">
              {([{value:'customer',label:'Customer',desc:'Browse & buy',icon:<ShoppingBag className="w-5 h-5"/>},{value:'vendor',label:'Vendor',desc:'Sell & list',icon:<Store className="w-5 h-5"/>}] as const).map(opt=>(
                <button key={opt.value} type="button" onClick={()=>setRole(opt.value)} className={`flex flex-col items-center gap-1 py-4 rounded-2xl border-2 font-semibold text-sm transition-all ${role===opt.value?'border-orange-500 bg-orange-50 text-orange-700 shadow-md shadow-orange-100':'border-gray-200 text-gray-500 hover:border-orange-200 hover:bg-orange-50/50'}`}>
                  {opt.icon}<span className="font-black">{opt.label}</span><span className="text-xs opacity-70">{opt.desc}</span>
                </button>
              ))}
            </div>
            <AnimatePresence>{error && <motion.div initial={{opacity:0,y:-8}} animate={{opacity:1,y:0}} exit={{opacity:0}} className="flex items-start gap-3 p-4 mb-5 rounded-xl bg-red-50 border border-red-200"><AlertCircle className="w-4 h-4 text-red-500 flex-shrink-0 mt-0.5"/><p className="text-red-600 text-sm font-medium">{error}</p></motion.div>}</AnimatePresence>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-gray-700 mb-1.5 uppercase tracking-wide">Full Name</label>
                <div className="relative"><User className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400"/><input type="text" required value={fullName} onChange={e=>setFullName(e.target.value)} className={`${ic} pl-10`} placeholder="Amara Johnson"/></div>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type={showPass?'text':'password'} required value={password} onChange={e=>setPassword(e.target.value)} className={`${ic} pl-10 pr-10`} placeholder="Min. 8 characters"/>
              ...
              <div>
                ...
                <input type={showPass?'text':'password'} required value={confirm} onChange={e=>setConfirm(e.target.value)} className={`${ic} ${confirm&&confirm!==password?'border-red-300':''}`} placeholder="Repeat password"/>
                {confirm&&confirm!==password&&<p className="text-xs text-red-500 mt-1 font-medium">Passwords don't match</p>}
              </div>
              <button type="submit" disabled={submitting} className="w-full py-3.5 bg-gradient-to-r from-orange-500 to-red-600 text-white rounded-xl font-black text-sm hover:from-orange-600 hover:to-red-700 transition-all shadow-lg shadow-orange-200 disabled:opacity-60 flex items-center justify-center gap-2">
                {submitting?<><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"/>Creating...</>:<>Create {role==='vendor'?'Vendor':'Customer'} Account <ArrowRight className="w-4 h-4"/></>}
              </button>
            </form>
            <p className="text-center text-sm text-gray-500 mt-5">Already have an account? <Link href="/auth/login" className="text-orange-500 font-bold hover:text-orange-700">Sign in</Link></p>
          </div>
        </div>
      </motion.div>
    </div>
...
export default function SignupPage() {
  return <Suspense fallback={<div className="min-h-screen pt-[64px] flex items-center justify-center bg-orange-50"><div className="w-10 h-10 border-4 border-orange-500 border-t-transparent rounded-full animate-spin"/></div>}><SignupInner/></Suspense>;
}
